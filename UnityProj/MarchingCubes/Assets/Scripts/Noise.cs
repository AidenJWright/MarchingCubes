...
namespace SimplexNoise
{
    ...
    public static class Noise
    {
        ...
        public static float[,,] Calc3D(int width, int height, int length, float scale, float xOffset, float yOffset, float zOffset)
        {
            ...
            for (var i = 0; i < width; i++)
                for (var j = 0; j < height; j++)
                    ...
                    {
                        float x = i * xOffset, y = j * yOffset, z = k * zOffset;
                        float val = Generate((i + xOffset) * scale, (j + yOffset) * scale, (k + zOffset) * scale) + 1;
                        float y2 = (j + yOffset) * scale;
                        val = Mathf.Lerp(
                            val,
                            y2/2.5f,
                            Mathf.Abs(y2-1.5f)
                        );
                        if(Input.GetKey(KeyCode.Backspace)) Debug.Log(y2);
                        values[i, j, k] = val;
                    }
            return values;
        }
        //(Generate((i + xOffset) * scale, (j + yOffset) * scale, (k + zOffset) * scale) * 128 + 128)
